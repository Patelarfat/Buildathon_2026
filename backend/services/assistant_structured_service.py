...
def build_structured_assistant_response(
    ...
    # =========================================================================
        ...
        # Check if a specific photo is mentioned in query (e.g. "photo 4", "photo #4")
        photo_match = re.search(r'photo\s*#?\s*(\d+)', query_lower)
        target_photo_id = int(photo_match.group(1)) if photo_match else None

        if target_photo_id:
            photo_obj = db.query(models.SitePhoto).filter(
                models.SitePhoto.id == target_photo_id,
                models.SitePhoto.project_id == project_id
            ).first()
            if photo_obj:
                photo_summary = get_project_ppe_summary(db=db, project_id=project_id, site_id=photo_obj.site_id, area_id=photo_obj.area_id)
                photo_people = [p for p in photo_summary["people"] if p.get("photo_id") == target_photo_id]
                if photo_people:
                    ai_workers_detected = len(photo_people)
                    ai_compliance_count = sum(1 for p in photo_people if p.get("compliant"))
                    ai_violations_count = sum(1 for p in photo_people if not p.get("compliant"))
                    ppe_pct = round((ai_compliance_count / ai_workers_detected * 100), 1) if ai_workers_detected > 0 else 100.0
                    violations_list = [f"Person {p.get('person_id')}: {', '.join(p.get('violations', []))}" for p in photo_people if not p.get("compliant")]
                    compliance_list = [f"Person {p.get('person_id')}: Fully PPE Compliant" for p in photo_people if p.get("compliant")]

        # Build violation breakdown by PPE type across all workers
        ...
        for p in all_workers:
            ...
            # Helper to check if item is detected/present
        ...
        for ph in project_photos:
            ...
            if not p_people and target_photo_id and ph.id != target_photo_id:
                continue
        ...
        if ai_violations_count > 0:
            ...
            insight = (
                f"Current PPE compliance is critically low at {ppe_pct}%. "
        ...
        if ai_workers_detected > 0:
            ...
            summary = "AI Computer Vision PPE analysis: No workers detected in latest site camera scans."
